...
class NetsuiteCustomRecordUpdateV1
  # ==== Parameters
  ...
  def execute
    ...
    begin
      puts "Fetching custom record." if @debug_logging_enabled
      # Fetch the existing custom record.
      ...
      puts "Updating custom record." if @debug_logging_enabled
      # Update the custom Record with the updated fields.
      response = record.update(
        custom_field_list: record.custom_field_list, 
        rec_type: NetSuite::Records::CustomRecord.new(internal_id: @type_id)
      )
