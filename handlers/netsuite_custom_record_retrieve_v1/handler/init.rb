...
class NetsuiteCustomRecordRetrieveV1
  # ==== Parameters
  ...
  def execute
    ...
    begin
      puts "Retrieving custom record with internal id of #{@internal_id}." if @debug_logging_enabled
      # Perform search
      response = NetSuite::Records::CustomRecord.get(
        type_id: @type_id,
    ...
    <<-RESULTS
    <results>
      <result name="Output">#{escape(response ? response.to_record.to_json : {})}</result>
